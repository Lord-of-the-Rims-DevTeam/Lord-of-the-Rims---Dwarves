...
namespace Dwarves
{
    //Currently just gold instead of silver.
    public class ItemCollectionGenerator_Treasure : ItemCollectionGenerator
    {
        protected override void Generate(ItemCollectionGeneratorParams parms, List<Thing> outThings)
        {
            int? count = parms.count;
            int num = (count == null) ? Rand.RangeInclusive(1, 3) : count.Value;
            float? totalMarketValue = parms.totalMarketValue;
            float num2 = (totalMarketValue == null) ? Rand.Range(1500f, 4000f) : totalMarketValue.Value;
            TechLevel? techLevel = parms.techLevel;
            TechLevel techLevel2 = (techLevel == null) ? TechLevel.Spacer : techLevel.Value;
        ...
        {
            if (Rand.Chance(0.5f) && (validator == null || validator(ThingDefOf.Gold)))
            {
            ...
            if (Rand.Chance(0.35f) && (from x in ItemCollectionGeneratorUtility.allGeneratableItems
                    where x.itemGeneratorTags != null &&
                          x.itemGeneratorTags.Contains(ItemCollectionGeneratorUtility.SpecialRewardTag) &&
                          (validator == null || validator(x)) && Mathf.Abs(1f - x.BaseMarketValue / value) <= 0.35f
            ...
            if (Rand.Chance(0.13f))
            {
                ...
                if ((from x in ItemCollectionGenerator_Meteorite.mineables
                    where x.building.isResourceRock &&
                          x.building.mineableThing.BaseMarketValue >= minExpensiveMineableResourceMarketValue &&
                          (validator == null || validator(x.building.mineableThing))
            ...
            ItemCollectionGenerator_Treasure.Option option2 =
            (from option in ItemCollectionGeneratorUtility.allGeneratableItems.Select(delegate(ThingDef td)
                {
                    if (td.techLevel > techLevel)
                    {
                        return null;
                    }
                    if (td.itemGeneratorTags != null &&
                        td.itemGeneratorTags.Contains(ItemCollectionGeneratorUtility.SpecialRewardTag))
                    {
                        return null;
                    }
                    if (!td.IsWithinCategory(ThingCategoryDefOf.Apparel) &&
                        !td.IsWithinCategory(ThingCategoryDefOf.Weapons) &&
                        !td.IsWithinCategory(ThingCategoryDefOf.Art) && (td.building == null || !td.Minifiable) &&
                        (td.tradeTags == null || !td.tradeTags.Contains("Exotic")))
                    {
                        return null;
                    }
                    ...
                    return new ItemCollectionGenerator_Treasure.Option
                    {
                        thingDef = td,
                        quality = ((!td.HasComp(typeof(CompQuality)))
                            ? QualityCategory.Normal
                            : QualityUtility.RandomQuality()),
                        stuff = stuff
                    };
                })
                where option != null
                select option).MinBy(delegate(ItemCollectionGenerator_Treasure.Option option)
            {
                float value2 =
                    StatDefOf.MarketValue.Worker.GetValue(
                        StatRequest.For(option.thingDef, option.stuff, option.quality), true);
